package routers

import (
	"encoding/json"
	"net/http"
	"time"

	"github.com/Christayc/redsocial_bankbook/bd"
	"github.com/Christayc/redsocial_bankbook/jwt"
	"github.com/Christayc/redsocial_bankbook/models"
)

/* Login realiza el ingreso del usaurio*/
func Login(w http.ResponseWriter, r *http.Request) {
	w.Header().Add("content-type", "applications/json")

	var t models.Usuario

	err := json.NewDecoder(r.Body).Decode(&t)
	if err != nil {

		http.Error(w, "Usuario y/o contraseña invalida, colocala correctamente."+err.Error(), 400)
		return
	}
	if len(t.Email) == 0 {
		http.Error(w, "El mail del usuario es requerido", 400)
		return
	}

	documento, existe := bd.IntentoLogin(t.Password, t.Email)
	if existe == false {
		http.Error(w, "Usuario y/o contraseña invalida, colocala correctamente", 400)
		return
	}
	jwtkey, err := jwt.GeneroJWT(documento)
	if err != nil {

		http.Error(w, "Ocurrio un error al intentar generar el TOKEN correspondiente."+err.Error(), 400)
		return
	}

	resp := models.RespuestaLogin{
		Token: jwtkey,
	}

	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusCreated)
	json.NewEncoder(w).Encode(resp)

	expirationsTime := time.Now().Add(24 * time.Hour)
	http.SetCookie(w, &http.Cookie{
		Name:    "token",
		Value:   jwtkey,
		Expires: expirationsTime,
	})

}
